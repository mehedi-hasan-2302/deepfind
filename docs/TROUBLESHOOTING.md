...
- DeepFind runs with the current user's normal permissions and does not bypass protected folders. Restore access using ordinary operating-system permissions or choose an accessible root; do not run the entire application as administrator merely to index more files. Inaccessible content remains unindexed, while metadata can remain searchable when it was readable.
- The extraction deadline bounds caller wait time and purges cancelled queued work, but it cannot forcibly terminate an in-process parser that ignores interruption. Such a parser can retain one bounded worker until it exits; restart DeepFind if parser capacity remains unavailable, and preserve the problematic document for a privacy-safe bug report.
