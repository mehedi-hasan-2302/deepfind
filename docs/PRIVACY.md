...
The Lucene search index is stored under `<data-directory>/index`. SQLite structured state is stored in `<data-directory>/deepfind.db`; `deepfind.db-wal` and `deepfind.db-shm` may exist while the application is running. Neither store is encrypted by DeepFind. The SQLite database contains local paths, scan failure descriptions, counters, and timestamps but not extracted document contents; those remain in Lucene. The history API is loopback-only and returns at most 100 jobs per request. No extraction temporary files are created by the current implementation. Application logs remain console-only and record job identifiers and exception classes, not document contents, private queries, or full paths. DeepFind will not claim its local state is encrypted until encryption is implemented and verified.
